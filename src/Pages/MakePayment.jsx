...
// --- নতুন লাইন: SweetAlert2 ইম্পোর্ট করুন ---
...
const MakePayment = () => {
    ...
    useEffect(() => {
        ...
        if (currentRent > 0 && stripe && elements) {
            ...
            axiosSecure.post('/create-payment-intent', { price: currentRent })
                .then(res => {
    ...
    const handleApplyCoupon = async () => {
        ...
        if (isNaN(currentRent) || currentRent <= 0) {
            setCouponError('Please enter a valid rent amount before applying a coupon.');
            return;
        }
        ...
        try {
            ...
            const validCoupon = coupons.find(c => c.couponCode === couponCode && c.available);

            if (validCoupon) {
                const discountAmount = currentRent * (validCoupon.discountPercentage / 100);
    ...
    if (isUserError) {
        return <div className="text-center text-red-500 mt-8">Error loading user data: {userError.message}</div>;
    }
        ...
        return (
            <div className="p-6 bg-gray-50 min-h-screen flex items-center justify-center">
                <div className="bg-white p-8 rounded-lg shadow-md text-center">
                    <p className="text-xl font-semibold text-gray-700">You are not a member or do not have a rented apartment yet.</p>
                    <p className="text-gray-500 mt-2">Please become a member or apply for an agreement to make payments.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            <h2 className="text-3xl font-bold text-center mb-10 text-gray-800">Make a Payment</h2>

            <div className="bg-white p-8 rounded-lg shadow-md max-w-lg mx-auto">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    {/* Read-only Fields from Rented Apartment Info */}
                    <div>
                        <label className="label">Member Email</label>
                        <input type="text" {...register('memberEmail')} className="input input-bordered w-full" readOnly />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label className="label">Floor</label>
                            <input type="text" {...register('floor')} className="input input-bordered w-full" readOnly />
                        </div>
                        <div>
                            <label className="label">Block Name</label>
                            <input type="text" {...register('blockName')} className="input input-bordered w-full" readOnly />
                        </div>
                        <div>
                            <label className="label">Apartment No.</label>
                            <input type="text" {...register('apartmentNo')} className="input input-bordered w-full" readOnly />
                        </div>
                    </div>

                    {/* Original Rent - Now an editable input */}
                    <div>
                        <label className="label">Original Rent</label>
                        <input
                            type="number"
                            step="0.01"
                            {...register('rent', { required: 'Rent amount is required', min: { value: 0.01, message: 'Rent must be greater than 0' } })}
                    ...
                    <div className="flex items-end gap-2">
                        ...
                        <button type="button" onClick={handleApplyCoupon} className="btn btn-secondary flex items-center gap-2">
                    ...
                    {appliedCoupon && (
                        <p className="text-green-600 text-md font-semibold mt-2">
                            Applied: "{appliedCoupon.couponCode}" - {appliedCoupon.discountPercentage}% Discount!
                        </p>
                    )}

                    {/* Display Final Rent after Discount */}
                    {parseFloat(manualInputRent) > 0 && (
                        <div className="mt-4 p-4 bg-blue-50 rounded-md">
                            <p className="text-lg font-semibold text-gray-800">
                                Final Amount to Pay: <span className="text-blue-600">${discountedRent.toFixed(2)}</span>
                                {appliedCoupon && (
                                    <span className="text-sm text-gray-500 ml-2 line-through">${parseFloat(manualInputRent).toFixed(2)}</span>
                                )}
                            </p>
                        </div>
                    )}
                    ...
                    <div>
                        <button
                            ...
                            disabled={!stripe || !elements || parseFloat(manualInputRent) <= 0 || !watch('month') || isProcessing || !clientSecret}
                        >
                            <FaCreditCard />
