// src/Pages/Dashboard/ManageCoupons.jsx
import React, { useState } from 'react';
import { useForm } from 'react-hook-form';
// import useAxiosSecure from '../../Hooks/useAxiosSecure';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { toast } from 'react-toastify';
import { FaPlus, FaToggleOn, FaToggleOff, FaEdit } from 'react-icons/fa'; // Icons
import useAxiosSecure from '../Hooks/useAxiosSecure';
...
const ManageCoupons = () => {
    ...
    const addCouponMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries(['allCoupons']); // ডেটা রিফেচ করুন
    ...
    // কুপন অ্যাভেইলেবিলিটি আপডেট করার মিউটেশন
    const updateCouponAvailabilityMutation = useMutation({
        mutationFn: async ({ id, available }) => {
            const res = await axiosSecure.patch(`/api/coupons/${id}/availability`, { available });
            return res.data;
        },
        onSuccess: () => {
        ...
        onError: (err) => {
            toast.error(err.response?.data?.message || 'Failed to update coupon availability.');
        }
    });
    ...
    const onSubmitAddCoupon = (data) => {
        addCouponMutation.mutate({
            ...data,
            discountPercentage: parseInt(data.discountPercentage), // নিশ্চিত করুন যে এটি সংখ্যা
            available: true // ডিফল্টভাবে নতুন কুপন উপলব্ধ থাকবে
        });
    };
    ...
    if (isError) {
        return <div className="text-center text-red-500 mt-8">Error loading coupons: {error.message}</div>;
    }

    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <h2 className="text-3xl font-bold text-center mb-10 text-gray-800">Manage Coupons</h2>
                            ...
                            <tbody>
                                {coupons.map((coupon, index) => (
                                    <tr key={coupon._id}>
                                        ...
                                        <td>
                                            <span className={`badge ${coupon.available ? 'badge-success' : 'badge-error'}`}>
                                        ...
                                        <td>
                                            <button
                                                ...
                                                className={`btn btn-sm ${coupon.available ? 'btn-warning' : 'btn-success'} text-white`}
                                                disabled={updateCouponAvailabilityMutation.isLoading}
                                            >
                                                {coupon.available ? <FaToggleOff /> : <FaToggleOn />} {coupon.available ? 'Set Unavailable' : 'Set Available'}
                                            </button>
                                        </td>
                                    </tr>
                                ))}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md mx-4">
                        ...
                        <form onSubmit={handleSubmit(onSubmitAddCoupon)} className="space-y-4">
                            <div>
                                ...
                                />
                                {errors.couponCode && <p className="text-red-500 text-sm mt-1">{errors.couponCode.message}</p>}
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                ></textarea>
                                {errors.couponDescription && <p className="text-red-500 text-sm mt-1">{errors.couponDescription.message}</p>}
                            </div>
                            <div className="flex justify-end gap-4 mt-6">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="btn btn-ghost">
                                    Cancel
                                </button>
                                <button type="submit" className="btn btn-primary" disabled={addCouponMutation.isLoading}>
                                    Add Coupon
                                </button>
                            </div>
                        </form>
