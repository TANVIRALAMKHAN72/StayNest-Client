...
import { Link, useLocation, useNavigate } from 'react-router'; // সঠিক import
...
const Login = () => {
 ...
 const handleGoogleLogin = async () => {
  try {
    ...
    // তোমার ব্যাকএন্ড ইউজার API এর URL (local বা production)
    const backendUrl = 'http://localhost:3000/users';

    // ইউজার ডাটা যা পাঠাবে
    const savedUser = {
      name: user.displayName,
      email: user.email,
      photoURL: user.photoURL,
      role: 'user'  // নতুন ইউজার হলে default role 'user'
    };

    // প্রথমে চেক করে নাও ইউজার আগে থেকে আছে কি না
    // যদি না থাকে, তাহলে POST; থাকলে skip বা PUT করো

    // নিচে শুধু POST দিচ্ছি, তুমি চাইলে GET করে চেক করতে পারো
      ...
      <div className="w-full max-w-md p-8 shadow-md bg-white rounded-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            />
            {errors.password && (
              <span className="text-red-500">{errors.password.message || "Password must be at least 6 characters"}</span>
            )}
          </div>
