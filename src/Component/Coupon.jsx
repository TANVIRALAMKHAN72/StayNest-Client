...
import useAxiosSecure from '../Hooks/useAxiosSecure'; // Make sure this path is correct
      ...
      <div className="max-w-5xl mx-auto text-center">
        <Slide direction="up" triggerOnce>
          <h2 className="text-3xl md:text-4xl font-bold mb-6 text-gray-900 flex items-center justify-center gap-2">
            <MdDiscount className="text-indigo-500 text-4xl" /> Exclusive Coupons
          </h2>
          <p className="text-gray-600 mb-10 text-lg">Grab the best deals for your next stay!</p>
        </Slide>
        {coupons.length === 0 ? (
            <p className="text-gray-700 text-xl mt-8">No coupons available at the moment. Check back later!</p>
        ) : (
            <Swiper
                    ...
                    <Fade triggerOnce direction="up" cascade damping={0.3}>
                      <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200 hover:shadow-xl transition-all duration-300">
                        <div className="text-indigo-600 text-xl font-semibold mb-2 flex items-center gap-2">
                          ...
                          {coupon.discountPercentage ? `${coupon.discountPercentage}% OFF` : 'Discount'}
                        </div>
                        {/* এখানে পরিবর্তন করা হয়েছে: `couponDescription` ব্যবহার করা হয়েছে */}
                        <p className="text-gray-700 mb-3">{coupon.couponDescription || 'No description provided.'}</p>
                        {/* পরিবর্তন শেষ */}
                        <div className="flex items-center justify-between bg-gray-100 px-4 py-2 rounded-md">
                          <span className="font-mono text-sm text-gray-800">{coupon.couponCode}</span>
                          <button
                        ...
                        {coupon.available !== undefined && (
                            <p className={`text-sm mt-1 ${coupon.available ? 'text-green-500' : 'text-red-500'}`}>
                                Status: {coupon.available ? 'Available' : 'Used/Unavailable'}
                            </p>
                        )}
                      </div>
                    </Fade>
